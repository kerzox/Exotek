...
public class ConveyorBeltEntity2 extends BasicBlockEntity implements IServerTickable, IClientTickable, IConveyorBeltCollision {
    ...
    public ConveyorBeltEntity2(BlockPos pos, BlockState state) {
        super(Registry.BlockEntities.CONVEYOR_BELT_ENTITY.get(), pos, state);
    }

    public List<ConveyorBeltItemStack> touchingMiddleOfBelt() {
        ...
        items = level.getEntitiesOfClass(ConveyorBeltItemStack.class,
                new AABB(x + 6 / 16f, y, z + 6 / 16f, x + 8 / 16f, y + 10 / 16f, z + 8 / 16f),
...
//                    }
//                }
                if (level.getBlockEntity(pos.relative(getBeltDirection())) instanceof ConveyorBeltEntity2 belt) {
                    stop = belt.stop;
                } else {
    ...
    private void setConveyorStatus(boolean stop) {
        for (ConveyorBeltItemStack conveyorBeltItemStack : touchingMiddleOfBelt()) {
            if (!stop) setDeltaMovementFromConveyorBeltSpeed(conveyorBeltItemStack);
            else conveyorBeltItemStack.setDeltaMovement(Vec3.ZERO);
        }
        this.stop = stop;
    }
